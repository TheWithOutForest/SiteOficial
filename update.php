...
if (isset($_GET['cpf'])) {
    ...
    $stmt->bind_param("s", $cpf); // "s" indica que o CPF é uma string
    $stmt->execute();
    ...
    if ($_SERVER['REQUEST_METHOD'] == 'POST') {
        ...
        $sql = "UPDATE cliente SET nome = ?, endereco_cliente = ?, telefone = ? WHERE cpf = ?";
        $stmt = $conn->prepare($sql);
        $stmt->bind_param("ssss", $nome, $endereco_cliente, $telefone, $cpf); // "ssss" indica que os parâmetros são strings
        $stmt->execute();
